...
def main(argv):
    ...
    SLEEP = (
        "import time, sys;print('START');sys.stdout.flush();"
        "time.sleep(3);print('STOP')"
    )
    SLEEPER = ["--arg=-c", "--arg", SLEEP]
    subprocess.check_call(
        [sys.executable, "-m", "ncolony", "ctl"]
        + DEFAULTS
    ...
    subprocess.check_call(
        [
            os.path.join(binLocation, "twistd"),
            "--logfile",
            LOG_FILE,
            "--pidfile",
            PID_FILE,
    ...
    subprocess.check_call(
        [sys.executable, "-m", "ncolony", "ctl"] + DEFAULTS + ["restart-all"]
    )
    print("sleeping for 5 seconds")
    time.sleep(5)
