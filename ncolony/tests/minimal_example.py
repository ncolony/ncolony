...
tempDir = os.path.join(here, 'build', '_example')
...
ctllib.add(places, name='example', cmd=twistd, args=['--logfile', root.child('example.log').path, '-ny', tac.path], extras=extras)
ctllib.add(places, name='beatcheck', cmd=twistd, args=['--logfile', root.child('beatcheck.log').path, '-n', 'ncolony-beatcheck', '--messages='+places.messages, '--config='+places.config])
args = (twistd, '--logfile='+root.child('master.log').path, '--pidfile='+root.child('twistd.pid').path, 'ncolony',
